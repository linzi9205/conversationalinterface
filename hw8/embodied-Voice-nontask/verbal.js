...
var response;
    var play_re = /sing\s(.+)/i;  // creating a regular expression
    ...
    if (play_parse_array && state === "initial") {
      response = "ok, singing " + play_parse_array[1];
    } else if (user_said.toLowerCase().includes("sing") && state === "initial") {
      response = "sing what?";
      state = "sing_song"
